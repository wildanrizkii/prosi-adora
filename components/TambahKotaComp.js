...
export function Namereducer(state, action) {
  if (action.type === "loading") {
    ...
    return {
      hasil: (
        <p className="help is-danger" style={{ fontSize: "15px" }}>
          nama kota tidak memenuhi ketentuan
        </p>
      ),
      warnaTextbox: "input is-danger",
      icon: <FontAwesomeIcon icon="times" color="red" />,
...
export function NamaKota({ className, value, onChange, icon, hasil }) {
  return (
    <div className="field">
      <label className="label">Nama kota</label>
      <div className="control has-icons-left has-icons-right">
        <input className={className} type="text" placeholder="Nama kota" value={value} onChange={onChange} />
