...
import { Modal, IsiModalSuccess, IsiModalFailed } from "../../../components/TambahJenisComp";
...
export default function JenisItem({ hasil }) {
    ...
    semuaAkun = hasil.map((x, index) => {
      return (
        ...
        >
          ...
          <td>
            <Link href={`JenisItem/Edit/${x.id_jenis}`} className="button is-success is-small">
              Edit
            </Link>
            {x.status === 1 ? (
              <button className="button is-danger is-small" style={{ marginLeft: "5px" }} onClick={() => changeStatus(x.id_jenis, false)}>
                Non-Aktifkan
              </button>
            ) : (
              <button className="button is-primary is-small" style={{ marginLeft: "5px" }} onClick={() => changeStatus(x.id_jenis, true)}>
                Aktifkan
              </button>
            )}
          </td>
  ...
  return (
    <>
      ...
      <Link className="button is-link" href="JenisItem/TambahJenis" style={{ marginBottom: "10px" }}>
        Tambah
      </Link>
...
export async function getServerSideProps() {
  const query = "select nama,kode,id_jenis,status from jenis";
  const values = [];
  try {
